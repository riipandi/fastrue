...
pub async fn send_email_smtp(
    ...
    let smtp_host = get_envar("FASTRUE_SMTP_HOST", Some("localhost"));
    let env_smtp_port = get_envar("FASTRUE_SMTP_PORT", Some("1025"));
    let smtp_username = get_envar("FASTRUE_SMTP_USERNAME", None);
    let smtp_password = get_envar("FASTRUE_SMTP_PASSWORD", None);
    let smtp_secure = get_envar("FASTRUE_SMTP_SECURE", None);
